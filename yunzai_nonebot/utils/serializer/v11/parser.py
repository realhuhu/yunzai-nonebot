...
async def message_parser(message: Iterator[ReceivableMessageSegment]):
...
async def sender_parser(sender: Sender):
...
async def reply_parser(reply: Reply):
...
async def anonymous_parser(anonymous: Anonymous):
